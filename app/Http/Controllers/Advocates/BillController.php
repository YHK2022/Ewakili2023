...
class BillController extends Controller
{
    ...
    public function get_bill_index()
    {
       ...
       public function get_payment_index()
    {
        if(Auth::check()){
            $user_id = Auth::user()->id;
            //echo $user_id;exit();
            ...
            //check bill
            if(Payment::where('profile_id', $profile_id)->exists()){
                $bills = Payment::where('profile_id', $profile_id)->orderBy('created_at', 'desc')->get();
            }else{
                $bills = "No data";
            }

            //dd($profile);exit;
            return view('advocates.payment.payment_index', [
                'petition_form' => $petition_form,
                'profile' => $profile,
                'profile_id' => $profile_id,
                'cur_year' => $cur_year,
